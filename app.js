...
document.getElementById('lmaois').addEventListener('load', function () { gsap.to("#attr a", { color: colors[4], duration: 1 }); })
...
function updateValues() {
  ...
  if (ScrollTrigger.isInViewport('#ghost') && document.getElementById('ghost').style.opacity == 0 && nscroll) { // you can use selector text
  ...
  else if (isInViewport(document.getElementById('rule')) && scr == true) {
...
gsap.to('#name', {
  backdropFilter: "blur(0px)", duration: 1, delay: 1, transform: "translate(-50%, -50%) scale(100%)", ease: "expo.out", onComplete: function () {
...
function abt(){
  nscroll = false
  gsap.to(window, { duration: 0.5, scrollTo: "#titlewe" });
  nscroll = true
}
function ct(){
  nscroll = false
  gsap.to(window, { duration: 0.5, scrollTo: "#rule2" });
...
function j(){
  nscroll = false
  gsap.to(window, { duration: 0.5, scrollTo: "#rule3" });
  if(ScrollTrigger.isInViewport('#name')){
  document.getElementById("bdiv").style.pointerEvents = 'none'
    if (window.mobileCheck() == false) {
      scr = true;
